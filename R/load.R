  ...
  if(!is.null(extraColumns)) {
    ...
    for(col_name in names(extraColumns)) {
      ...
      if(is.function(col_value)) {
        isomut[, (col_name) := vapply(isomut$file_name, col_value, character(1))]
      } else {
        if(length(col_value) == 1) {
          isomut[,(col_name) := col_value]
        } else {
          stopifnot(df$file_name %in% names(col_value))
          isomut[,(col_name) := col_value[isomut$file_name]]
        }
      }
    }
  }
...
#' @param anno path to annotation file in `gtf` or `gff` format or `GRanges`
...
#' @export
annotate_isomut <- function(isomut, ref = ref_scer, anno = txdb_scer,
                            noDups = FALSE, predictCoding = TRUE, ...) {
  ...
  ## load annotation
  stopifnot(is.character(anno) || class(anno, "GRanges"))
  if(is.character(anno)) {
    suppressMessages(
      txdb <- suppressWarnings(makeTxDbFromGFF(anno, metadata = "gene_id"))
    )
    gr <- import(anno)
  } else {
    suppressMessages(
      txdb <- suppressWarnings(makeTxDBFromGRanges(anno))
    )
    gr <- anno
  }
...
#'
#' @export
predict_coding <- function(isomut, ref = ref_scer, anno = txdb_scer,
